...
| ID | P | Gap | Needs |
|---|---|---|---|
| G-01 | P0 | **Phone-to-phone test** (PRD §74) | **You and one other person**, two devices, ideally separate networks. Step-by-step checklist ready in [PHONE_TEST.md](PHONE_TEST.md). The last MVP acceptance step; automation cannot answer whether it is fun. `e2e/two-phones.spec.ts` now covers the mechanical half — two mobile contexts, a shared co-op card, and real `setOffline` drops — but two emulated Chromiums share one network stack and one clock, so they cannot tell you whether two people on a café's wifi see each other promptly, whether an iOS socket survives an hour asleep, or whether anyone enjoys it. |
...
| G-22 | P3 | **Monetization** (PRD §59–60) | **Your business decision**, plus a Stripe account and keys. The PRD deliberately defers this until gameplay is proven. **The seam is now built** (G-28) — what is missing is a way to pay, not a way to be a supporter. **Paid-entry fundraisers with prizes are a separate matter with real legal exposure — read [FUNDRAISER_LEGAL.md](FUNDRAISER_LEGAL.md) before building any of it.** The likelier model is room-size pricing — see [PRICING.md](PRICING.md). |
...
| G-28 | P3 | Nothing knew what an account was *allowed* to do | **Entitlement seam** (migration 0027): a private `user_entitlements` table, `tier_for()`, and `entitlements_for_me()`, with each tier's allowance defined once in SQL and mirrored in `lib/entitlements.ts` under a test that they agree. Granting is a trusted path only — there are no write policies at all, so a Stripe webhook or `scripts/set-tier.js` writes it and nobody can grant themselves anything. It also **closed a real hole**: the per-user AI limit used to be sent by the caller. Never exploited, because our own route sent the right number, but a limit the caller chooses is not a limit. It now comes from the tier, and `p_user_limit` can only make it stricter. **2026-09-07: superseded by `platform.entitlements`**, a schema shared with couples-game (its own Phase 8 needed a richer, occasion-based entitlement model, and rather than building two systems the tier model moved onto the shared one). Staged in two migrations (`20260908000001_platform_schema.sql`, additive only; `20260908000002_platform_repoint.sql`, the actual behavior change — `tier_for()` repointed, identical signature) — both live on the shared database as of 2026-09-07, verified end to end (`tier_for`/`ai_hourly_limit_for`/`admin_grant_tier`/`admin_expire_tier` all round-trip correctly; zero changes needed to `app/api/ai/generate-squares/route.ts` or `lib/ai/rate-limit.ts`). `public.user_entitlements` stays in place as a safety net for now; there is no rush to drop it. See couples-game/docs/SHARED_BACKEND.md §3. |
